...
// NEW
// make sure they point to same page & read only
pde_t *
cowuvm(pde_t *pgdir, uint sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    if (!(*pte & PTE_P))
      panic("copyuvm: page not present");
    // NEW
    *pte &= ~PTE_W;          // set it to not writable, set the right bit to 0
    pa = PTE_ADDR(*pte);     // addr of page table in mmu.h 144, upper bits
    flags = PTE_FLAGS(*pte); // lower bits
    if (mappages(d, (void *)i, PGSIZE, pa, flags) < 0)
      goto bad;
    increment_refC(pa);
  }
  lcr3(PADDR(pgdir)); // NEW flushing tlb
  return d;

bad:
  freevm(d);
  lcr3(PADDR(pgdir)); // flushing tlb
  return 0;
}
...
void pgfltHandler(uint err_code)
{
  // cprintf("handing page fault\n");
  ...
  uint refCount = get_refC(pa);
  // cprintf("CoW: refCount %d\n", refCount);
  char *mem;
  // panic("haha");
  if (refCount > 1)
  {
    ...
    *pte = (pte_t)((uint)mem | PTE_P | PTE_U | PTE_W);
    // uint new_pa = PTE_ADDR(*pte);

    // decrement the reference count by 1
    decrement_refC(pa);
    // increment_refC(new_pa);
    // decrement - passed
  ...
  {
    *pte = (*pte | PTE_W); // set to writeable
  }
  else
  {
  ...
  // cprintf("Pagefault handling done\n");
